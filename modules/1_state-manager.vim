...
function s:modules.state_manager.InflateState()
    ...
    for path in paths
        try
            ...
            echo "This is a new project, please save first"
            return
        endtry
    endfor

    const state_manager = g:danvim.app_data.state_manager
    const tabs_titles = get(g:danvim.app_data, 'state_manager_tabs_titles', [])
    const highests_viewports = get(g:danvim.app_data, 'state_manager_highests_viewports', [])
    const pane_breaker = get(g:danvim.app_data, "state_manager_pane_breaker", [])

...
let s:base_vars = 
\ {
	\ "popup_marks_dir": [ expand($MY_VIM_MARKS_DIR), s:home_vim . "/popup.shortcuts" ],
	\ "dictionaries_dir": [ expand($MY_VIM_DICTS), s:home_vim . "/dictionaries" ],
	\ "bridge_file": "/tmp/bridge",
	\ "clipboard_commands": [ "wl-copy", "wl-paste" ],
	\ "workspaces_dir": [ expand($MY_VIM_WORKSPACES), s:workspaces ],
	\ "initial_workspace_tries": [ "all", "root", "basic", "workspaces", "core", "source" ],
	\ "loaders_dir": [ expand($MY_VIM_LOADERS_DIR), s:home_vim . "/loaders/trending" ],
	\ "initial_message": [ "DanVim loaded!" ],
	\ "basic_structure_initial_dir": [ expand($MY_VIM_INITIAL_DIR), s:home_vim . "/" ],
	\ "last": v:null
\ }
